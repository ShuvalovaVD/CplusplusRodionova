// Лекция 4. Наследование. Виртуальные функции.
...
Полиморфизм в языке программирования С++ обеспечивается за счет использования производных классов и виртуальных функций. Виртуальная функция — это функция, 
объявленная с ключевым словом virtual в базовом классе и переопределенная в одном или в нескольких производных классах. Виртуальные функции являются 
особыми функциями, потому что при вызове во время исполнения программы объекта производного класса с помощью указателя (ссылки) компилятор С++ определяет, 
какую функцию вызвать, основываясь на типе объекта, а не на типе используемого указателя. Для разных объектов могут вызываются разные версии одной и той
же виртуальной функции. Класс, содержащий одну или более виртуальных функций, является полиморфным классом. Виртуальная функция объявляется в базовом классе с
использованием ключевого слова virtual. Когда она переопределяется в производном классе, повторять ключевое слово virtual нет необходимости, 
хотя и в случае его повторного использования ошибки не возникнет.
*/

// Код Тани, решение её лабы 3
